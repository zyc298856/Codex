    ...
    : mLogger(logger), mInputShape(inputShape)
{
    ...
    //mContext->setInputShape("Input:0", inputShape);
    mContext->setInputShape("actual_input_1", inputShape);
...
float* MimoUnet::infer(const cv::Mat& img, float* outHostBuf) {
	...
	//setOutputName("Identity:0");
	setOutputName("output1");
	...
	cudaMemcpyAsync(mBuffers[inIdx]+sizeof(float)*mSizes[inIdx]/3, img.ptr<float>(),
					sizeof(float)*mSizes[inIdx]/3, cudaMemcpyHostToDevice, mStream);

	cudaMemcpyAsync(mBuffers[inIdx]+sizeof(float)*mSizes[inIdx]*2/3, img.ptr<float>(),
					sizeof(float)*mSizes[inIdx]/3, cudaMemcpyHostToDevice, mStream);
	...
	cudaMemcpyAsync(outHostBuf,mBuffers[outIdx],
	                sizeof(float)*mSizes[outIdx]/3,cudaMemcpyDeviceToHost,mStream);
...
int MimoUnet::inputWidth()const { return	mInputShape.d[2]; }

// 返回网络输入高度（H），即NCHW中的H值。
int MimoUnet::inputHeight()const { return	mInputShape.d[1]; }
